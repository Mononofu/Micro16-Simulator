...
object Micro16SimulatorGUI extends SimpleSwingApplication {
	val src = "micro16-code.txt"
	def top = new MainFrame {
		...
		contents = new BoxPanel(Orientation.Horizontal) {
			...
			val lineNumber = new ListView( (1 to 30).map("%02d \n".format(_))) {
				fixedCellHeight = 15
			...
			contents += new ScrollPane(state) {
				maximumSize = new Dimension(200, 4000)
			}
